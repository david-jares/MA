...
func StartRouter(router *echo.Echo) {
	log.Infof("Starting location-service on port %d.", port)

	go func() {
		if err := router.Start(":" + strconv.Itoa(port)); err != nil && !errors.Is(err, http.ErrServerClosed) {
			log.Fatalf("Shutting down location-service: %s", err.Error())
		}
	}()
	...
	log.Infof("Gracefully shutting down location-service")
